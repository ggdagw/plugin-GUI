...
#include "SimpleThresholdProcessor.h"
#include "SimpleThresholdProcessorEditor.h"

//If the processor uses a custom editor, it needs its header to instantiate it
//#include "ExampleEditor.h"


SimpleThresholdProcessor::SimpleThresholdProcessor()
  : GenericProcessor ("SimpleThreshold"), threshold(200.0) //, state(true)

{
    ...
    // Parameter 0 code
    auto parameter0 = new Parameter ("Threshold", "Threshold", 0, 2000, 200, 0);
...
*/
AudioProcessorEditor* SimpleThresholdProcessor::createEditor()
{
    editor = new SimpleThresholdProcessorEditor (this, true);
...
void SimpleThresholdProcessor::setParameter (int parameterIndex, float newValue)
{
...
void SimpleThresholdProcessor::process (AudioSampleBuffer& buffer)
{
    /**
...
void SimpleThresholdProcessor::saveCustomParametersToXml (XmlElement* parentElement)
{
    XmlElement* mainNode = parentElement->createNewChildElement ("SimpleThresholdProcessor");
...
void SimpleThresholdProcessor::loadCustomParametersFromXml()
{
    ...
    {
        if (mainNode->hasTagName ("SimpleThresholdProcessor"))
        {
            int parameterIdx = -1;
