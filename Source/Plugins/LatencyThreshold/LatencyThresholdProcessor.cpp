...
#include "LatencyThresholdProcessor.h"
#include "LatencyThresholdProcessorEditor.h"

//If the processor uses a custom editor, it needs its header to instantiate it
//#include "ExampleEditor.h"


LatencyThresholdProcessor::LatencyThresholdProcessor()
  : GenericProcessor ("LatencyThreshold"), highThreshold(5000.0), lowThreshold(230.0), counting(false), sampleCount(0)

{
    ...
    // Parameter 0 code
    auto parameter0 = new Parameter ("HighThreshold", "HighThreshold", 0, 6000, 5000, 0);
    parameters.add (parameter0);

    // Parameter 1 code
    auto parameter1 = new Parameter ("LowThreshold", "LowThreshold", 0, 2000, 230, 1);
...
*/
AudioProcessorEditor* LatencyThresholdProcessor::createEditor()
{
    editor = new LatencyThresholdProcessorEditor (this, true);
...
void LatencyThresholdProcessor::setParameter (int parameterIndex, float newValue)
{
    ...
    std::cout << "old value for highThreshold: " << highThreshold << std::endl;
    std::cout << "old value for lowThreshold: " << lowThreshold << std::endl;
    if (parameterIndex == 0)
...
void LatencyThresholdProcessor::process (AudioSampleBuffer& buffer)
{
    /**
  ...
  const int timeOut = 10000; 
    ...
    for (int chan = 0; chan < nChannels; ++chan)
    {
        ...
        {
          if (*(bufPtr + n) > highThreshold) {
            counting = true;
	    sampleCount = 0;
          }
	  else if (*(bufPtr + n) > lowThreshold) {
	    *(bufPtr + n) = float(sampleCount)/100.0;
...
void LatencyThresholdProcessor::saveCustomParametersToXml (XmlElement* parentElement)
{
    XmlElement* mainNode = parentElement->createNewChildElement ("LatencyThresholdProcessor");
...
void LatencyThresholdProcessor::loadCustomParametersFromXml()
{
    ...
    {
        if (mainNode->hasTagName ("LatencyThresholdProcessor"))
        {
            int parameterIdx = -1;
